...
- **TypeScript only**, latest version. No JS files.
- **Yarn Workspaces** monorepo. Use `yarn workspace <name>` for package operations.
- **oxlint** for linting, **oxformat** or **biome** for formatting. No ESLint.
- **No Pinia/Vuex** in frontend. Use composables + provide/inject for state.
- **UnoCSS only** for styling. No `.css` or `.scss` files.
- **Kysely** for database queries. No raw SQL except in migrations.
- **Hono** for HTTP routing. No Express/Fastify.
- **Vitest** for all tests. Consistent test runner across packages.
...
Every stage of delivery MUST pass `yarn validate` before merging or proceeding to the next stage.

**Quality Gate pipeline** (runs in order, fails fast):

1. **Lint** — `yarn lint` (oxlint passes with no errors)
2. **TypeCheck** — `yarn typecheck` (tsc --noEmit passes)
3. **Test** — `yarn test` (vitest run passes)

**Rules:**
- `yarn validate` runs all three checks sequentially and stops on first failure
- Code MUST NOT be merged or delivered if `yarn validate` fails
- Format check: `yarn format:check` (biome) should also pass before merge
- Fix failures in order: lint → format → typecheck → test
