...
router.post("/config/reload", (c) => {
  return c.json({
    ...
    data: {
      models_count: 7,
      providers_count: 3,
      auths_count: Array.from(providersStore.values()).reduce(
        (sum, p) => sum + (p.auths?.length || 0),
        0,
      ),
      ...
      return c.json(
        {
          error: {
            message:
              "Rate limit exceeded. Max 10 requests per minute for this key.",
    ...
    choices: [
      {
        ...
        message: {
          role: "assistant",
          content:
            "Hello! I'm a mock AI assistant. This response was generated by the mock API server for testing purposes. How can I help you today?",
        },
        finish_reason: "stop",
      },
    ],
